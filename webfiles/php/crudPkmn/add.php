...
if($connexion){
    ...
    $execResult = $connexion->query("CREATE DATABASE IF NOT EXISTS 'crud-pokemon'");
    ...
    $execResult = $connexion->query("CREATE TABLE IF NOT EXISTS 'pokemon_games'(
        ...
        'pkmn_img' varchar(255) DEFAULT NULL,
        PRIMARY KEY ('pkmn_id'),
        UNIQUE KEY 'pkmn_id' ('pkmn_id'))");

        
if(isset($date) && isset($supp) && isset($titre) && isset($generation) && isset($url)){
    $execResult = $connexion->query("INSERT INTO  pokemon_games (pkmn_game_name, pkmn_generation, pkmn_release_date, pkmn_support, pkmn_img) VALUES ('$pkmn_game_name', $pkmn_generation, '$pkmn_release_date', '$pkmn_support', '$pkmn_image')"); ?>
